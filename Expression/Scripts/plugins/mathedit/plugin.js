						...
						[
							{
								...
								type : 'html',
								html : "<span class='mq-root-block mq-empty' id='" + currId + "' ></span>",
								setup: function (image) {
								    
								    var dialEl = jQuery(CKEDITOR.document.getById(currId).$);
								    
								    $('.cke_dialog_ui_vbox.cke_dialog_page_contents').css('height', '100%');
								    $('.cke_dialog_ui_vbox.cke_dialog_page_contents table').css('height', '100%');
								    var mathField = MQ.MathField(dialEl[0]);
								    mathField.reflow();
								    //MQ.StaticMath($(currId)[0]);
								    ...
								    mathField.latex(image.getAttribute('title'));
									//dialEl.mathquill('latex', );
								},
